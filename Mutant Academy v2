...
<title>Mutant Academy — Danger Room (V2)</title>
<style>
:root{
...
body{
  ...
  radial-gradient(1200px 600px at 10% 10%, rgba(110,231,183,0.04), transparent 6%),
  radial-gradient(800px 400px at 90% 90%, rgba(124,193,255,0.03), transparent 6%),
...
.app{
  width:100%; max-width:720px; background:linear-gradient(180deg, rgba(255,255,255,0.02), rgba(255,255,255,0.015));
...
.card{background:linear-gradient(180deg, rgba(255,255,255,0.012), rgba(255,255,255,0.008)); border-radius:10px;padding:12px;border:1px solid rgba(255,255,255,0.03)}
...
.avatar{width:68px;height:68px;border-radius:10px;background:rgba(255,255,255,0.02);display:flex;align-items:center;justify-content:center;font-size:34px}
.hpbar{height:12px;background:#071219;border-radius:8px;overflow:hidden;border:1px solid rgba(255,255,255,0.03)}
.fill{height:100%;transition:width .35s linear;background:linear-gradient(90deg,#46d0b9,#1aa08c)}
...
<body>
<div class="app" role="application" aria-label="Mutant Academy Danger Room v2">
  <div class="header">
    <div class="title">
      <div class="logo">MA</div>
      <div>
        <h1 class="h1">Mutant Academy — Danger Room</h1>
        <div class="hint">Choisis ton pouvoir et ta difficulté, passe les épreuves et affronte le simulateur final.</div>
      </div>
    </div>
    <div class="badge">v2 — futuriste</div>
  </div>
  ...
  <div class="grid">
      ...
      <div class="card" id="startCard">
        <div style="display:flex;justify-content:space-between;align-items:center">
          <div>
            ...
            <div style="margin-top:8px" class="small">Chaque pouvoir est **usage unique** durant l’entraînement + combat final.</div>
          </div>
        ...
        <div class="note">Version V2 — mode Danger Room. Les PV, dégâts et l'aléa dépendent de la difficulté.</div>
      </div>
    ...
    <div class="rightCol">
      ...
      <div class="card" style="margin-top:12px">
        ...
        <div class="note" style="margin-top:8px">Pouvoirs : usage unique. Pense à économiser ou utiliser au bon moment.</div>
      </div>
    </div>
  </div>
...
/* ================= DATA ================= */
const POWERS = {
  telekinesis:{id:'telekinesis',name:'Télékinésie',emoji:'🧠',desc:'Augmente l’esquive ennemi (30%) 1 tour',usage:false},
...
const VILLAINS = [
  ...
  {id:'sentinel',name:'sentinel',emoji:'🤖',pv:44,dmg:[4,9],note:'Attaques lourdes mais lent'}
];
...
let state = {
  ...
  enemy:null,
  score:0,
  stage:0, // 0..2 training, 3 final done
  challenges:[], // array of 'combat'|'rescue'|'puzzle'
  trainingRounds:0
};
...
function log(msg, cls=''){
  const d=document.createElement('div'); d.className='small'; d.innerHTML=msg;
...
function beginSession(){
  ...
  state.player.powerAvailable = true; state.chosenPower.usage=false;
...
function startTrainingRescue(){
  ...
  for(let i=1;i<=3;i++){
    const b = document.createElement('button'); b.className='actionBtn'; b.textContent='Porte '+i;
...
function resolveQuiz(choice){
  ...
  const q = state._quiz;
  if(choice === q.a){
    log('🟢 Bonne réponse — tactique améliorée.');
    state.score += 6;
  } else {
    log('🔴 Mauvaise réponse — perte de temps, légère fatigue.');
...
function renderCombat(isTraining){
  ...
  useBtn.onclick = ()=>usePower();
  // if training: limit rounds not strictly; we continue until one side tombe
}
...
function playerAttack(kind){
  ...
  // enemy may have armor-like effect
  if(state.enemy && Math.random()<0.08 && state.difficulty==='difficile'){ // small random resilience
    dmg = Math.max(0,dmg-2); log('🔰 Résilience adverse réduit les dégâts.');
  }
  if(state.enemy){
    ...
    updateEnemyUI();
    if(state.enemy.burn && state.enemy.burn>0){ /* handled each turn */ }
  } else {
    log('ℹ️ Pas d’ennemi présent.');
  }
  // scoring small reward
  if(state.enemy && state.enemy.name.includes('Robot')) state.score += 2;
  // if player had extraAction, consume extraAction (do not auto enemy react now)
  if(state.player.extraAction){
    state.player.extraAction = false; log('🔁 Action supplémentaire utilisée, l’ennemi ne réagit pas encore.');
    updatePlayerUI(); return;
  }
  // enemy reacts after short delay
  setTimeout(()=>enemyReact(),400);
}
...
function usePower(){
  ...
  // apply effects
  if(p.id==='telekinesis'){
    // give 30% evade applied to enemy attacks next turn (we store on player.evade)
    state.player.evade += 0.3;
  } else if(p.id==='claws'){
    state.player.clawsBoost = true;
  } else if(p.id==='speed'){
    // allow immediate extra action: lets player act again before enemy reaction
    state.player.extraAction = true;
  } else if(p.id==='ice'){
    if(state.enemy) state.enemy.frozen = true;
  } else if(p.id==='pyro'){
    if(state.enemy){ state.enemy.burn = 2; log('🔥 Brûlure : 2 tours'); }
  }
  // mark power used
  state.player.powerAvailable = false; state.chosenPower.usage = true;
  document.getElementById('btnUsePower').disabled = true;
}

/* enemy react */
function enemyReact(){
  if(!state.enemy) { // no enemy: move on
    log('ℹ️ Aucun ennemi — épreuve terminée.');
    state.stage++; setTimeout(()=>nextChallenge(),600);
    return;
  }
  // if enemy frozen skip and unfreeze
  if(state.enemy.frozen){
    log(`❄️ ${state.enemy.name} est gelé et rate son action !`);
    state.enemy.frozen = false;
    // apply burn tick if any
    if(state.enemy.burn && state.enemy.burn>0){
      state.enemy.pv -= 3; state.enemy.burn--; log(`🔥 Brûlure inflige 3 PV à ${state.enemy.name}`); spawnFloatingOver(document.getElementById('enemyFill'),'-3','#ffb86b'); updateEnemyUI();
    }
    checkCombatEnd();
    return;
  }
  // enemy attacks
  const base = rand(state.enemy.dmg[0], state.enemy.dmg[1]);
  // consider player's evade
  const evade = state.player.evade || 0;
  const reduced = Math.round(base * evade);
  const final = Math.max(0, base - reduced);
  ...
  // consume evade (one use)
  state.player.evade = 0;
  // enemy burn tick
  if(state.enemy.burn && state.enemy.burn>0){
    state.enemy.pv -= 3; state.enemy.burn--; log(`🔥 Brûlure : ${state.enemy.name} subit 3 PV`); spawnFloatingOver(document.getElementById('enemyFill'),'-3','#ffb86b'); updateEnemyUI();
  }
  updatePlayerUI();
  checkCombatEnd();
}
